...
public class AsyncTaskManage {
    ...
    public ThreadInfo registerThread(){
        ...
        if ( DEBUG )
            EvLog.i(TAG, "registerThread -->开始:tId="+currentThread.getId()+";tHCode="+currentThread.hashCode());
        synchronized(lock(currentThread.getId())){
            try{
                ...
                if ( DEBUG )
                    EvLog.i(TAG, "registerThread -->结束:tId="+currentThread.getId()+";tHCode="+currentThread.hashCode());
            }
        }
    }
    /**
    ...
    public void unregisterThread(){
        Thread currentThread = Thread.currentThread();
        if ( DEBUG )
            EvLog.i(TAG, "unregisterThread -->开始:tId="+currentThread.getId()+";tHCode="+currentThread.hashCode());
        synchronized(lock(currentThread.getId())){
            try{
                ...
                unlock(currentThread.getId());
                if ( DEBUG )
                    EvLog.i(TAG, "unregisterThread -->结束:tId="+currentThread.getId()+";tHCode="+currentThread.hashCode());
            }
        }
    }
    /**
    ...
    public int registerHttpTask(IAsyncTask httpTask){
        Thread currentThread = Thread.currentThread();
        if ( DEBUG )
            EvLog.i(TAG, "registerHttpTask -->开始:tId="+currentThread.getId()+";tHCode="+currentThread.hashCode());
        ...
        synchronized(lock(currentThread.getId())){
            try{
                ...
                unlock(currentThread.getId());
                if ( DEBUG )
                    EvLog.i(TAG, "registerHttpTask -->结束:result="+result+";tId="+currentThread.getId()+";tHCode="+currentThread.hashCode());
            }
        }
    }
    ...
    public void cancelAsyncTask(final long threadId,final int threadHashCode){
        if ( DEBUG )
            EvLog.i(TAG, "cancelAsyncTask -->开始:tId="+threadId+";tHCode="+threadHashCode);
            ...
            try{
                ...
                if(threadInfo != null){
                    if(threadInfo.isRegister() && !threadInfo.isCancel()){
                        ...
                        if(asyncTask != null){
                            EvLog.i(TAG, "cancelAsyncTask -->执行onCancel():tId="+threadId+";tHCode="+threadHashCode);
                            asyncTask.onCancel();
                ...
                unlock(threadId);
                if ( DEBUG )
                    EvLog.i(TAG, "cancelAsyncTask -->结束:tId="+threadId+";tHCode="+threadHashCode);
            }
        }
    }
