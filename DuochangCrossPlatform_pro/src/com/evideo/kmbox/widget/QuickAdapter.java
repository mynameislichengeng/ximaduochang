...
//    private static final String TAG = QuickAdapter.class.getSimpleName();
    ...
    public View getView(int position, View convertView, ViewGroup parent) {
        if (getItemViewType(position) == 0) {
            final BaseAdapterHelper helper = BaseAdapterHelper.get(context, convertView, parent, layoutResId, position);
            convert(helper, getItem(position), position);
            View view = helper.getView();
            view.setId(position);
            if(position > 0) {
                view.setNextFocusUpId(position - 1);
            }
            if(position < (getCount() - 1)) {
                view.setNextFocusDownId(position + 1);
            }
